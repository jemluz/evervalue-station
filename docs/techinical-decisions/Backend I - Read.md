...
- **SATS Calculation**: Coingecko does not always return `sats` as stable `vs_currency` for all tokens. Having real BTC/USD price allows Backend I - Read to calculate SATS in fail-safe way:
  - 1 BTC = 100,000,000 SATS
  - So Backend I - Read does: `(EVA_Price_in_BTC * 100,000,000)` to guarantee SATS value, without depending on API rounding.
